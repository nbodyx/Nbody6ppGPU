...
*       Remove ghosts from perturber neighbour lists.
      JOLD(1) = ICM
      CALL NBCHANGE(JLIST,4,JOLD,1,LIST,1,NTOT)
C      CALL NBREM(ICM,4,NP)
*
...
*       Compare global name & subsystem name to restore the mass.
  ...
  112     CONTINUE
          ...
          DO 115 K = 1,3
              ...
              X0(K,J) = X(K,J)
  115     CONTINUE
*       Set t0 to current time to avoid wrong prediction of x and xdot later
          T0(J) = TIME
  120 CONTINUE
*
...
*       Replace ICM in perturber neighbour lists by all subsystem members.
      JOLD(1) = ICM
      CALL NBCHANGE(JOLD,1,JLIST,4,LIST,1,NTOT)
C      CALL NBREST(ICM,4,NP)
*
...
*     remove ghost I from ghost list
