...
*
*     Remove ICM first
      IF(TIME+TOFF.NE.0.D0) CALL delay_remove_tlist(icm,step,dtk)
...
*     Add c.m. into NXTLST
      IF(TIME+TOFF.NE.0.D0) CALL delay_store_tlist(ICM)
*
*       Form perturber list.
      CALL KSLIST(IPAIR)
*
...
*       Estimate an appropriate KS slow-down index for G < GMIN.
          IF (KZ(26).GT.0.AND.STEP(ICM).GT.TK) THEN
              IMOD = INT(1 + LOG(STEP(ICM)/TK)/0.69)
              IMOD = MIN(IMOD,5)
          END IF
      END IF
*
...
*       Check diagnostic output of new hard binary.
     ...
     &                  GAMMA(IPAIR), RI
   90     FORMAT ('NEW H-BINARY Time[NB] ',1P,E25.16,0P,'  NAME(I1) '
