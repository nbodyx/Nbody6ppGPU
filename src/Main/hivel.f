...
*       Reduce membership and remove any distant member from the list.
     ...
     &                      SQRT(VL2)
    3         FORMAT (' HIVEL REMOVE    Time[NB] I NAME RI(NB) VI(NB) ',
     &             1P,E26.17,0P,2I12,1P,2E20.11,0P)
              CALL FLUSH(29)
              end if
              NHI = NHI - 1
              LISTV(1) = LISTV(1) - 1
...
*       Add any new high-velocity particles (save F**2 > N & STEP < DTMIN).
      NHV = 0
      DO 10 I = I1,I2
*       avoid check ghost particle
         if(body(I).eq.0) go to 10
         CALL JPRED(I,TIME,TIME)
...
*       Check maximum membership and possible ghost particle.
    ...
    9         FORMAT (' HIVEL ADD NS/KS/CHAIN  Time[NB] NHI I NAME ',
     &             'K* VI[NB] RI[NB] STEP[NB] ',1P,E16.7,0P,I6,2I12,I4,
     &             1P,3E16.7,0P)
              CALL FLUSH(29)
              end if
          END IF
...
*       Include single particles without further tests.
     ...
     &                       SQRT(RI2), STEP(I1)
   22         FORMAT (' HIVEL ADD SINGLE  Time[NB] NHI NAME IPHASE ',
     &             'VI[NB] RI[NB] STEP[NB] ',1P,E16.7,0P,I6,I12,I3,
     &             1P,3E16.7,0P)
              GO TO 30
          END IF
*       Evaluate two-body energy.
          I2 = JLIST(2)
          CALL JPRED(I2,TIME,TIME)
          RIJ2 = 0.0
          VIJ2 = 0.0
          RDOT = 0.0
          DO 25 K = 1,3
...
*       Accept outwards hyperbolic motion arising from recent interaction.
   ...
   28         FORMAT (' HIVEL ADD HYP  Time[NB] NHI NAME(I1) NAME(I2) ',
     &             'IPHASE RIJ[NB] ',1P,E16.7,0P,I6,2I12,I3,1P,E16.7,0P)
          END IF
      END IF
*
   30 CONTINUE
