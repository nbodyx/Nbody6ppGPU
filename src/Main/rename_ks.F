...
#ifdef SIMD
          IF (J.GE.IFIRST) FLAG_SEND = .true.
#endif

 39       IF (J.GE.IFIRST-2) THEN
*     Check replacing of single KS component by corresponding c.m.
 37          IF(LIST(1,J).GT.0.AND.LIST(2,J).LT.IFIRST) THEN
                JNEW = KVEC(LIST(2,J)) + N
                NNB = LIST(1,J)
                DO L = 2,NNB+1
                   IF (L.LE.NNB.AND.LIST(L+1,J).LT.JNEW) THEN
                      LIST(L,J) = LIST(L+1,J)
                   ELSE
                      LIST(L,J) = JNEW
#ifdef SIMD
...
*     Update neighbor list in AVX/SSE library
         IF(TIME.NE.0.0D0.AND.FLAG_SEND) THEN
*     --04/20/14 11:32-lwang-debug--------------------------------------*
***** Note:------------------------------------------------------------**
c$$$            IF(J.EQ.7660.and.rank.eq.0) THEN
c$$$               PRINT*,'J',J,'N',NAME(J),'LIST',LIST(1:LIST(1,J)+1,J),
c$$$     &          'IFIRST',IFIRST
c$$$               call flush(6)
c$$$            end if
