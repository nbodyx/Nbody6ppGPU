      SUBROUTINE CHECKL(I,NNB,XI,XIDOT,RS2,FIRR,FREG,FD,FDR,KLIST)
*
*
...
*       Now see whether body #J has already been included as neighbour.
  309 DO 310 L = 1,NNB
          IF (J.EQ.KLIST(L+1)) GO TO 308
  310 CONTINUE
*       Include body #J in sequential location of neighbour list.
      L = NNB + 1
  312 IF (J.GT.KLIST(L)) GO TO 314
      KLIST(L+1) = KLIST(L)
...
*       Advance list index by two for every identified pair.
      IF (KLIST(L).GT.IFIRST + 3.OR.L.GT.NNB + 1) GO TO 330
*
...
*
      LJ = NNB + 1
  327 IF (J.GT.KLIST(LJ)) GO TO 328
...
*       First use large increments to speed up the search.
      ...
      IF (KLIST(LG).LT.JBODY.AND.LG.LT.NNB + 1) GO TO 336
*
      LG = LG + 1
  338 LG = LG - 1
      IF (KLIST(LG).GT.JBODY.AND.LG.GT.2) GO TO 338
      IF (KLIST(LG).EQ.JBODY) ICASE = ICASE + KTIME
