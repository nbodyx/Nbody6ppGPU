      SUBROUTINE FPCORR(I,NBLOSS,NBGAIN,XI,XIDOT,KLIST)
*
*
...
C      REAL*8 FIRR(3),FREG(3),FD(3),FDR(3)
C      REAL*8 FMPI(3),FDMPI(3)
*
#ifdef GPU      
      INTEGER KLIST(2*LMAX)
#else
      INTEGER KLIST(NMAX)
#endif      
*
...
*       Initialize the derivative corrections.
      DO 70 K = 1,3
          ...
          SAVE2(K) = 0.0
          SAVE3(K) = 0.0
C          FMPI(K) = 0.5D0*(FREG(K) + FIRR(K))
C          FDMPI(K) = ONE6*(FDR(K) + FD(K))
   70 CONTINUE
*
