      SUBROUTINE CHMOD_CHAIN(ISUB,KCASE)
*
*
...
*       Bypass test and repeated diagnostics for large perturbation.
          ...
          RDX = 0.0
*     --10/29/13 13:58-lwang-bug-fix------------------------------------*
***** There is an unfortunate mixup in this routine of dir Nchain.
***** Namely the local chain coordinates and velocities are used
***** instead of the global values. On the positive side, this part is
***** only executed for relative perturbation between 0.05 and 0.5 when 
***** decision-making will be affected. I am sorry to say this defect
***** has been present for a long time (at least since 2006).
***** The revised routine performs intruder check on all chain members
***** (case NCH > 3).
***** -----------------------------------------------------------------**
          RJX = 1.0
...
*       See whether the intruder is approaching a chain member.
 ...
 3        CONTINUE
c$$$          L = 0
c$$$*       Evaluate actual distance and relative radial velocity of end member.
c$$$    3     RJX2 = (XCH(L+1) - X(1,JCLOSE))**2 +
c$$$     &           (XCH(L+2) - X(2,JCLOSE))**2 +
c$$$     &           (XCH(L+3) - X(3,JCLOSE))**2
c$$$          RDI = (XCH(L+1) - X(1,JCLOSE))*(VCH(L+1) - XDOT(1,JCLOSE)) +
c$$$     &          (XCH(L+2) - X(2,JCLOSE))*(VCH(L+2) - XDOT(2,JCLOSE)) +  
c$$$     &          (XCH(L+3) - X(3,JCLOSE))*(VCH(L+3) - XDOT(3,JCLOSE))
c$$$*       See whether any chain member is approaching the intruder.
c$$$          IF (RDI.LT.0.0) THEN
c$$$              RJX = SQRT(RJX2)
c$$$              RDX = RDI/RJX
c$$$          END IF
c$$$*       Consider the last chain member similarly.
c$$$          IF (L.EQ.0) THEN
c$$$              L = 3*(NN - 1)
c$$$              GO TO 3
c$$$          END IF
*     --10/29/13 13:59-lwang-end-bug-fix--------------------------------*
...
*       Check option for increasing or decreasing regularization parameters.
  ...
  700             FORMAT (' INCREASE   Time[NB] KSMAG GPERT RMIN ',
     &                 'RIJ[NB] ',1P,E15.6,0P,I4,1P,3E15.6)
                  CALL FLUSH(77)
              END IF
  ...
  705         FORMAT ('   REDUCE   Time{NB] KSMAG GPERT RMIN RIJ[NB] ',
     &             1P,E15.6,0P,I4,1P,3E15.6)
              CALL FLUSH(77)
          END IF
      END IF
*
...
*          if(rank.eq.0) print*,'TBLOCK =',tblock,'Time=',time
*          call flush(6)
c$$$          IF(TIME.LT.TBLOCK) THEN
c$$$             DTTB = TBLOCK - TIME
c$$$             IF (DTTB.GT.1.0) then
c$$$                print*,'Warning! Tblock - Time larger than 1.0'
c$$$                call flush(6)
c$$$                IDTTB = INT(DTTB)
c$$$                DITTB = DTTB - FLOAT(IDTTB)
c$$$                call STEPK(DITTB,DTTBN)
c$$$                DTTBN = DTTBN + FLOAT(IDTTB)
c$$$             else 
c$$$                call STEPK(DTTB,DTTBN)
c$$$             end if
c$$$             TBLOCK = TBLOCK - DTTBN
c$$$          END IF
...
*       Note that arithmetic mean tends to delay escape.
      ...
      IF (RM.GT.RESC.AND.RDOT.GT.0.0) THEN
          IF (RDOT**2.LT.2.0*BODY(ICH)/RI) THEN
*     --10/29/13 13:53-lwang-bug-fix------------------------------------*
***** Note: BODY(IESC) here is incorrect-------------------------------**
C*              FAC2 = 2.0*BODY(IESC)/(BODY(ICH) - BODYC(IESC))
             FAC2 = 2.0*BODYC(IESC)/(BODY(ICH) - BODYC(IESC))
*     --10/29/13 13:53-lwang-end----------------------------------------*
...
*     --10/29/13 14:09-lwang-bug-fix------------------------------------*
***** Note:update in 10/26/13 in Sverre's Email------------------------**
C*     &            (NN.EQ.3.AND.GI.GT.0.1)) THEN
     &             (RSUM.LT.5.0*RMIN.AND.GPERT.LT.0.01)) THEN
*     --10/29/13 14:10-lwang-end----------------------------------------*
                  KCASE = 0
                  GO TO 60
              END IF
