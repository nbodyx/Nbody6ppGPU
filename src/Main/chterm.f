...
*       With the new TLIST method, here time should be equal to TBLOCK.
...
*       Print diagnostics for high-velocity chain escaper after COLL or COAL.
   ...
   41     FORMAT (' CHAIN DISRUPT (high-velocity escaper):  NAME ',
     &         'NCH ECH[NB] V[km/s] ',I7,I4,F8.4,F6.1)
      END IF
*
...
*     Remove ghost from ghost list
          call delay_remove_tlist(I3,STEP,DTK)
          CALL FPOLY1(I3,I3,0)
          IF (NCH.GE.4) THEN
*     Remove ghost from ghost list
              call delay_remove_tlist(I4,STEP,DTK)
              CALL FPOLY1(I4,I4,0)
              CALL FPOLY2(I4,I4,0)
*     Add I4 into DELAY LIST
              call delay_store_tlist(I4)
              IF (NCH.GE.5.AND.I5.GT.0) THEN
*     Remove ghost from ghost list
                  call delay_remove_tlist(I5,STEP,DTK)
                  CALL FPOLY1(I5,I5,0)
                  CALL FPOLY2(I5,I5,0)
*     Add I5 into DELAY LIST
                  call delay_store_tlist(I5)
                  IF (NCH.EQ.6) THEN
*     Remove ghost from ghost list
                      call delay_remove_tlist(I6,STEP,DTK)
                      CALL FPOLY1(I6,I6,0)
                      CALL FPOLY2(I6,I6,0)
*     Add I6 into DELAY LIST
                      call delay_store_tlist(I6)
                  END IF
              END IF
          END IF
          CALL FPOLY2(I3,I3,0)
*     Add I3 into DELAY LIST
          call delay_store_tlist(I3)
          STEP3 = STEP(I3)
          STEP4 = STEP(I4)
*
...
*     Remove ghost from list
         ...
         call delay_remove_tlist(JCOMP,STEP,DTK)
          CALL KSREG
...
*       Rectify KS elements of dominant and weakly perturbed binary.
   ...
   72                 FORMAT (' CHAIN RECTIFY [All in NB]: ',
     &                     'NCH NSTEP EB/ECH SEMI GAMMA ERR ',
...
*     Add icomp and jcomp into DELAY LIST
          call delay_store_tlist(ICOMP)
          call delay_store_tlist(JCOMP)
      END IF
*
...
*       Initialize the perturbers to improve derivatives (same locations).
      ...
      DO 95 L = 2,NBC1
          ...
          DO 94 K = 1,3
              ...
              X0(K,J) = X(K,J)
   94     CONTINUE
*     Remove perturber index first
          call delay_remove_tlist(j,step,dtk)
          CALL FPOLY1(J,J,0)
          CALL FPOLY2(J,J,0)
*     Add index after renew step
...
*     --03/07/14 23:16-lwang-note---------------------------------------*
***** Note: Same reason as in subint
